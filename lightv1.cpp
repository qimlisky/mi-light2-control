...
// ===== Device config =====
const char* LAMP_IP   = "ipaddress";
const char* TOKEN_HEX = "your-token";//你的token
// DID: the string from the Mi Home app, used as-is in JSON "did" field
const char* DEVICE_DID = "device-id";//米家 App 中查看
// =========================
...
const DWORD    SOCKET_TIMEOUT_MS = 5000;   // raised to 5s

// Set to 1 to print raw JSON send/recv (for debugging)
#define DEBUG 1
...
std::vector<uint8_t> build_packet(const std::string& json,
    ...
    std::vector<uint8_t> cs(16+16+enc.size());
    memcpy(cs.data(),   hdr,         16);
    memcpy(cs.data()+16,token.data(),16);
    memcpy(cs.data()+32,enc.data(),  enc.size());
    auto cksum = md5(cs.data(),cs.size());
...
void do_handshake(Session& s) {
    ...
    if (sendto(s.sock,(const char*)hello,32,0,(sockaddr*)&s.addr,sizeof(s.addr))==SOCKET_ERROR)
    ...
    if (r<32) throw std::runtime_error("handshake timeout / bad response");
...
std::string send_cmd(Session& s, const std::string& json) {
    ...
    if (r==SOCKET_ERROR){
        ...
        throw std::runtime_error("recv failed WSA="+std::to_string(e));
    }
    ++s.ts;

    if (r<32||resp[0]!=0x21||resp[1]!=0x31)
        throw std::runtime_error("bad response header");
    if (r==32) return "(empty - heartbeat only)";
...
// did MUST be the string from the Mi Home app, NOT derived from packet header
...
// Simple scalar extractor: find "key":VALUE in flat JSON
std::string jval(const std::string& j, const std::string& key) {
    auto needle = "\"" + key + "\":";
    ...
    char c=j[p];
    size_t e;
    if (c=='"'){ e=j.find('"',p+1); return j.substr(p+1,e-p-1); }
    e=j.find_first_of(",}]",p);
    return j.substr(p,e-p);
}

// Extract value for a given piid from get_properties response
std::string piid_val(const std::string& j, int piid) {
    std::string needle = "\"piid\":" + std::to_string(piid);
    auto p = j.find(needle);
    while (p != std::string::npos) {
        // find next "value": within the same object (before next "piid")
        auto vp = j.find("\"value\":", p);
        auto np = j.find("\"piid\":",  p+1);
        if (vp==std::string::npos || vp>np) { p=np; continue; }
        vp += 8;
...
void print_help() {
    std::cout <<
        ...
        "  info            - query device info (miIO.info)\n"
        "  quit            - exit\n";
}
...
int main() {
    ...
    while(true){
        ...
        try {
            // Re-handshake before every command to get fresh timestamp
            do_handshake(s);

            if (cmd=="quit"||cmd=="exit") break;
            else if (cmd=="on")     { auto r=cmd_set(s,1,"true");  std::cout<<"  "<<r<<"\n"; }
            else if (cmd=="off")    { auto r=cmd_set(s,1,"false"); std::cout<<"  "<<r<<"\n"; }
            else if (cmd=="bright") {
                int v=0; iss>>v;
                if(v<1||v>100){std::cout<<"  range: 1-100\n";continue;}
                auto r=cmd_set(s,2,std::to_string(v)); std::cout<<"  "<<r<<"\n";
            }
            else if (cmd=="ct") {
                int v=0; iss>>v;
                if(v<2700||v>6500){std::cout<<"  range: 2700-6500\n";continue;}
                auto r=cmd_set(s,3,std::to_string(v)); std::cout<<"  "<<r<<"\n";
            }
            else if (cmd=="status") { auto r=cmd_get(s);  print_status(r); }
            else if (cmd=="info")   { auto r=cmd_info(s); std::cout<<"  "<<r<<"\n"; }
